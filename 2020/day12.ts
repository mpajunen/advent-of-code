...
const act1: Reducer<Ship, Action> = (ship, { action, value }) => {
  ...
  switch (action) {
    ...
    case 'W':
      return {
        facing,
        position: Vec2.move(position, { dir: action, length: value }),
      }
    case 'L':
    case 'R':
    ...
    case 'F':
      return {
        facing,
        position: Vec2.move(position, { dir: facing, length: value }),
      }
  }
}
...
const act2: Reducer<WithWaypoint, Action> = (state, { action, value }) => {
  ...
  switch (action) {
    ...
    case 'W':
      return {
        position,
        waypoint: Vec2.move(waypoint, { dir: action, length: value }),
      }
    case 'L':
    case 'R':
    ...
    case 'F':
      return {
        position: Vec2.add(position, Vec2.mul(waypoint, value)),
        waypoint,
      }
  }
}
...
export default (rows: string[]) => {
  const moves: Action[] = rows.map(r => ({
    action: r[0] as Act,
    value: parseInt(r.slice(1)),
  }))

  const ship1: Ship = { position: Vec2.origin, facing: 'E' }
  const state: WithWaypoint = {
