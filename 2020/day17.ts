...
type Key<D extends Dimension = 4> = D extends 4
  ? 'x' | 'y' | 'z' | 'w'
  : D extends 3
    ? 'x' | 'y' | 'z'
    : D extends 2
...
type State<D extends Dimension = 4> = D extends 4
  ? Cell[][][][]
  : D extends 3
    ? Cell[][][]
    : D extends 2
...
const createCycle =
  <D extends Dimension>(dimension: D): Cycle<D> =>
  state => {
    ...
    const getRange = (values: unknown[]): number[] =>
      List.range(-1, values.length + 1)
...
const listSum =
  <T>(get: GetValue<T>): GetValue<T[]> =>
  values =>
    Num.sum(values.map(get))

const getGetCount = <D extends Dimension>(dimension: D): GetValue<State<D>> =>
  List.range(0, dimension).reduce(
    sub => listSum(sub) as GetValue,
    (n => n) as GetValue,
  )
