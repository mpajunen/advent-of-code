...
const occupiedBy = (options: Readonly<Occupant[]>) => ([, occupant]: Cell) =>
...
const between = (a: number, b: number) => ([{ x }]: Cell) =>
...
const findAmphipodMoves = (maze: Maze) => (cell: Cell<Amphipod>): Move[] => {
...
const moveAmphipod = (before: Maze, { from, to, amphipod }: Move): Maze =>
  before.map(
    ([p, o]) => [p, Vec2.equal(p)(from) ? OPEN : Vec2.equal(p)(to) ? amphipod : o],
  )
...
const findSolutions = (maze: Maze) => {
  ...
  while (incomplete.length > 0) {
    ...
    minSolutionCost = Math.min(minSolutionCost, ...newSolutions.map(s => s.cost))
