...
const playQuantumTurn = ([game, count]: Entry): Entry[] =>
  DIRAC_ROLL_COUNTS.map(([move, moveCount]) => [
    playTurn(game, move),
    count * moveCount,
  ])
...
const combineIdenticalGames = (games: Entry[]): Entry[] =>
  Counts.fromEntries(
    games.map(([game, count]) => [JSON.stringify(game), count]),
  )
    .entries()
...
const playQuantum = (game: Game): Counts<number> => {
  ...
  while (activeGames.length > 0) {
    ...
    finished.forEach(([game, count]) =>
      winCounts.add(findWinner(game, WIN_SCORE), count),
    )
