...
const getSumFunction = filter => {
    const getCollectionSum = collection => filter(collection) ? collection.reduce(getSum, 0) : 0

    const getNodeSum = node => typeof node === 'number' ? node
        : (typeof node === 'string' ? 0 : getCollectionSum(node))
...
const isNotMapWithRed = collection => !(immutable.Map.isMap(collection) && collection.includes('red'))
