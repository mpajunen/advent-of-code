...
const hasNoInvalidLetters = nums => nums
    .filter(num => invalidLetters.indexOf(num) !== -1)
    .length === 0

const hasIncreasingStraight = nums => nums
    .map((value, key, nums) => nums[key + 1] === value + 1 && nums[key + 2] === value + 2)
    .includes(true)

const getFirstPairIndex = nums => nums
    .reduce((index, value, key, nums) =>
        index === -1 && nums[key + 1] === value
            ? key
            : index
    , -1)
