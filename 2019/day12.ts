...
const sum = (a: Vec3, b: Vec3): Vec3 => ({
  x: a.x + b.x,
  y: a.y + b.y,
  z: a.z + b.z,
})

const magnitude = ({ x, y, z }: Vec3): number =>
...
const getGravityUnit = (source: Vec3, target: Vec3): Vec3 => ({
  x: compare(source.x, target.x),
  y: compare(source.y, target.y),
  z: compare(source.z, target.z),
})

const applyMoonGravity = (
  { id, position, velocity }: Moon,
  moon: Moon,
): Moon => ({
  id,
  position,
...
const energy = ({ position, velocity }: Moon): number =>
  magnitude(position) * magnitude(velocity)
...
const getMaxCounts = (
  allCounts: Record<number, number>[],
...
const factorProduct = (counts: Record<number, number>): number =>
  Object.entries(counts).reduce(
    (value, [num, count]) => value * parseInt(num) ** count,
    1,
  )
