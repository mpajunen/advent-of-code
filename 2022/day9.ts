...
const covered = (visited: Vec2[]) =>
  new Set(visited.map(p => `${p.x},${p.y}`)).size

export default (rows: string[]) => {
  const steps = getInput(rows).flatMap(({ dir, length }) =>
    List.repeat<Vec2>(Vec2.screenUnits[dir], length),
  )

  const headPositions = List.scan(Vec2.origin, steps, Vec2.add)

  const result1 = covered(getNextPositions(headPositions))
  const result2 = covered(
    List.range(1, 10).reduce(getNextPositions, headPositions),
  )
