...
const getInput = (rows: string[]) => {
  ...
  const moves = moveRows
    .map(
      Input.parseByPattern<[number, number, number]>('move %i from %i to %i'),
    )
...
export default (rows: string[]) => {
  ...
  const result1 = Object.values(moved1)
    .map(s => s[0])
    .join('')
  const result2 = Object.values(moved2)
    .map(s => s[0])
    .join('')
