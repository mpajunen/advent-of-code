...
const parse = Input.parseByPattern<[number, number, number, number]>('Sensor at x=%i, y=%i: closest beacon is at x=%i, y=%i')
...
const getInput = (rows: string[]) => {
  ...
  const sensors = positions.map(p => ({ ...p.sensor, strength: Vec2.manhattan(p.sensor, p.beacon) }))
...
const splitArea = ({ min, max }: Area): Area[] => {
  const middle = { x: Math.trunc((max.x + min.x) / 2), y: Math.trunc((max.y + min.y) / 2) }
...
const findUncoveredPositions = (sensors: Sensor[]) => (area: Area): Vec2[] => {
...
export default (rows: string[]) => {
  ...
  const result1 = Num.sum(getTestCoverage(sensors)) - getTestBeaconCount(beacons)
