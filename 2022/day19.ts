...
const parse = Input.parseByPattern<number[]>('Blueprint %i: Each ore robot costs %i ore. Each clay robot costs %i ore. Each obsidian robot costs %i ore and %i clay. Each geode robot costs %i ore and %i obsidian.')
...
const getBlueprint = (row: number[]): Blueprint => ({
  ...
  robotCosts: {
    ...
    obsidian: [{ resource: 'ore', amount: row[3] }, { resource: 'clay', amount: row[4] }],
    geode: [{ resource: 'ore', amount: row[5] }, { resource: 'obsidian', amount: row[6] }],
  },
})
...
const getGeodeMax = (timeMax: number) => (blueprint: Blueprint): number => {
  ...
  const getTimeToAccumulate = (s: State) => ({ amount, resource }: Cost) =>
    s.robots[resource] === 0
      ? UNAVAILABLE_TIME
  ...
  const getStateResourceValue = (s: State) => (resource: Resource) =>
    (s.resources[resource] + (timeMax - s.time) * s.robots[resource]) * RESOURCE_VALUES[resource]
  const getStateValue = (state: State) =>
    -Num.sum(RESOURCES.map(getStateResourceValue(state)))

  const getNextStates = (state: State): State[] => [wait(state), ...buildRobotOptions(state)]
  ...
  const getBestStates = (states: State[]): State[] => {
    ...
    const all = states.flatMap(s => s.time === minTime ? getNextStates(s) : s)
