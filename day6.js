...
const splitAction = action => action
    .split(' ')
    .slice(-4)
const convertLoc1 = location => location
    .split(',')
    .map(x => parseInt(x, 10))
...
const applyAction = appliers => (previous, action) =>
    appliers[action](previous)
const isInArea = (i, j) => (start, end) =>
    i >= start.x && i <= end.x && j >= start.y && j <= end.y
const applyActionMaybe = appliers => checkArea => (previous, {action, start, end}) =>
    checkArea(start, end)
        ? applyAction(appliers)(previous, action)
        : previous
const reduceLocation = (appliers, initial) => (i, j) =>
    actions.reduce(
        applyActionMaybe(appliers)(isInArea(i, j)),
        initial
    )
