...
export const charCount = (char: string, text: string): number =>
  text.split(char).length - 1

export const charCounts = (s: string): Record<string, number> =>
  List.counts(s.split(''))
...
export const map = (
  ...
  str: string,
): string => str.split('').map(transform).join('')
...
export const splitAt = (s: string, index: number) => [
  s.slice(0, index),
  s.slice(index),
]

export const splitIn = (s: string, chunkCount: number) =>
  chunk(s.length / chunkCount, s)
