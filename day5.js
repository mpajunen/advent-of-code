...
const disallowedPairs = ['ab', 'cd', 'pq', 'xy']
    .map(pair => pair.split(''))
...
const isPreviousSame = (char, index, word) => index !== 0 && word[index - 1] === char
const isPairBad = (current, previous) => disallowed.has(previous) && disallowed.get(previous) === current
const isPreviousBad = (char, index, word) => index !== 0 && isPairBad(char, word[index - 1])
...
const getPair = (char, index, word) => index > 0 ? word[index - 1] + char : null
...
const isGapRepeater = (char, index, word) => index > 1 && word[index - 2] === char
